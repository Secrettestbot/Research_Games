...
function runPatternMemory(participantId, condition) {
    ...
    class CardFlipPlugin {
        ...
        trial(display_element, trial, jsPsych) {
            ...
            const showCard = () => {
                const cardIcon = trial.card_type === 'ACE' ? '🂡' : '⬜';
                const cardClass = trial.card_type === 'ACE' ? 'card-ace' : 'card-blank';
                const cardLabel = trial.card_type === 'ACE' ? 'ACE' : 'BLANK';

                const html = `
                    <div style="text-align: center; padding: 40px;">
                        <div style="margin-bottom: 20px; color: #7f8c8d;">
                            Card ${trial.trial_number} of ${trial.total_trials}
                        </div>

                        <div id="card-container" style="cursor: pointer;">
                            ${flipped ? `
                                <div class="card-display">
                                    <div class="${cardClass}">
                                        ${cardIcon}
                                ...
                                ${trial.show_feedback ? `<div style="font-size: 24px; font-weight: bold; margin-top: 20px;">${cardLabel}</div>` : ''}
                            ` : `
                                <div class="card-back">
                                    <span>?</span>
                                </div>
                                <div style="margin-top: 20px; font-size: 18px; color: #7f8c8d;">
                ...
                if (!flipped) {
                    const cardContainer = display_element.querySelector('#card-container');
                    cardContainer.addEventListener('click', handleFlip);
                } else {
                    setTimeout(() => endTrial(), CONFIG.feedback_time);
                }
            };
            ...
            const endTrial = () => {
                ...
                const timeToFlip = flipTime ? flipTime - startTime : null;

                jsPsych.finishTrial({
                    card_type: trial.card_type,
                    flipped: flipped,
                    rt: rt,
                    time_to_flip: timeToFlip
                });
            };
    ...
    jsPsych.plugins['card-flip'] = CardFlipPlugin;
